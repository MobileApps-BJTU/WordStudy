...
public class makeNotify extends BroadcastReceiver{
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if(intent.getAction().equals("android.intent.action.BOOT_COMPLETED")){
            ...
            OOB.setNotify(settings.getString("time", "18:00 afternoon"),context);
        }
            ...
            if(settings.getBoolean("notify", false)){
                ...
                if(notify){
                   // NotificationManager notiManager = (NotificationManager) context.getSystemService("配置");
                    Notification notification = new Notification(R.drawable.icon, "words need to review", System.currentTimeMillis());
                    notification.flags = Notification.FLAG_AUTO_CANCEL;
                    Intent intent1 = new Intent(context, Main.class);
                    PendingIntent contentIntent = PendingIntent.getActivity(context, 0,
                            intent1, PendingIntent.FLAG_UPDATE_CURRENT);
                    notification.setLatestEventInfo(context, "review", "words need to review",
                            contentIntent);
